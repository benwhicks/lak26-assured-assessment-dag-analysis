...
merged_nodes <- tdag_s1 %N>% as_tibble() |> 
    ...
    mutate(node_broad_group = str_extract(node_group, "^.{1}"))
...
merged_all_graph_layout_adjusted <- 
    ...
    mutate(
        # x = if_else(name == "Post.Fb",x + 20,x),
        y = if_else(name == "Post.Fb",y + 4,y)
           )

ggraph(graph = merged_all_graph_layout_adjusted) + # great layout for DAGs
    geom_edge_link(aes(alpha = edge_w), 
...
ggraph_with_layout <- function(g, lyt = merged_all_graph_layout_adjusted) {
...
p1 <- ggraph_with_layout(tdag_s1)
p2 <- ggraph_with_layout(tdag_s2)
p3 <- ggraph_with_layout(tdag_s3)
p4 <- ggraph_with_layout(tdag_s4)
p5 <- ggraph_with_layout(tdag_s5)
p6 <- ggraph_with_layout(tdag_s6)
p7 <- ggraph_with_layout(tdag_s7)
p8 <- ggraph_with_layout(tdag_s8)
