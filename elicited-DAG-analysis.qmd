...
Idea - a function to compute an edge score for each edge in all the graphs. 
- Score from 0 to 1 based on the proportion of times the edge is included, WHEN both nodes are present.
- This is then multiplied by -1 IF there is disagreement about the direction (might need to be sufficient? i.e. if only 1 of 8 thinks it's the other way then this might be a bit extreem)
- Would also be nice to have if the path is in the set of backdoor paths
...
edges_with_metrics <- 
    edges_with_metrics |> 
    left_join(switches |> select(from, to) |> mutate(switched = "contested")) |> 
...
### Time spent with model and modelling utility
...
p_time_and_representaion
ggsave("time-modelling-and-representation.png", width = 6, height = 3, dpi = 600)
```
...
pMergedFineLegend <- merged_all_graph |> 
    g_plot_fine() + 
...
pMergedFineLegend
pMergedFineLegendTall <- merged_all_graph |> 
    g_plot_fine() + 
    theme(legend.position = "right") +
...
pMergedFineLabelled <- pMergedFine + 
    geom_node_text(
        ...
        data = merged_all_graph_layout |>  filter(node_w > 0), repel = TRUE, max.overlaps = 4) +
...
#### Building main DAG figure

``` {r}
pAll3by3 <- (p2peerHighlight | p5 | p7) / (p4peerHighlight | p1 | p8) / (p3peerHighlight | leg_tall_plot | p6)
pFineTogetherLabelled <- pAll3by3 / pMergedFineLabelled + plot_layout(heights = c(1,1,1,3))
...
ggsave("dags_fine_all_and_merged_labelled.png", pFineTogetherLabelled, width = 9, height = 10, dpi = 900)
...
p_top_node_pagerank_no_outcome <- 
    all_node_groups_aggregated_metrics |> 
    group_by(node_group) |>
    mutate(p_traced_group = mean(p_traced)) |>
    ungroup() |> 
    ...
    geom_boxplot(aes(fill = p_traced_group), colour = "lightgray", size = 0.3, alpha = 0.7) +
    geom_point(aes(colour = p_traced), alpha = 0.9) +
    scale_color_gradient(low = friendly_pals$vibrant_seven[[1]], high = friendly_pals$vibrant_seven[[6]],
                         breaks = c(0,1),
                         labels = c("Untraced","Traced"),
                         name = "") +
    scale_fill_gradient(low = friendly_pals$vibrant_seven[[1]], high = friendly_pals$vibrant_seven[[6]]) +
    theme_minimal() +
    ylab("Node group") +
...
ggsave("node_pagerank_no_outcome.png", p_top_node_pagerank_no_outcome, width = 10, height = 4, dpi = 600)
```
...
``` {r}
#| eval: false
node_groups_table |> clipr::write_clip()
```
